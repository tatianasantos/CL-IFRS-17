# General functions for the article "CONFIDENCE LEVEL ASSESSMENT FOR IFRS 17 WITH THE NP APPROXIMATION"

# Function to determine the percentile for the LIC and LRC at a defined confidence level
# CL is the confidence level specified by the user
# Remaining arguments defined based on the other functions
...
cmom_cp <- function(D, Vt, H){ 
  J = length(Vt)
  mom = matrix(data=0, nrow=J, ncol=3)
  for(s in 1:3){
    for(i in 1:J){
      for(t in (i-1):(J-1)){
        mom[i,s] = mom[i,s] + D^(s*t)*Vt[t+1]*H[[s]] #need to adjust the index for Vt since t starts at zero
      }
    }
  }
  return(mom)
...
# This function assumes that we have an estimate for theta
...
mom_ibnr_gen <- function(D, Pj, PId, mom_theta, cmom){
  ...
  for(j in 1:J){
    ...
    mom[3] = mom[3] + C(D, Pj, PId, cmom)[j]*mom_theta[[j,1]] +
      ...
      (A(D, Pj, PId, cmom)[j])^3*mom_theta[[j,3]]
  }
  return(mom)
}
...
mom_cbni_gen <- function(D, PJ, PId, mom_theta, cmom){
  ...
  for(j in 1:J){
    ...
    mom[3] = mom[3] + C_J(D, PJ, PId, cmom)[j]*mom_theta[[j,1]] +
      ...
      (A_J(D, PJ, PId, cmom)[j])^3*mom_theta[[j,3]]
  }
  return(mom)
}
