...
# We have already estimated the delay probabilities based on development factors and claim rates
# Method of moments
...
LogLik_A <- function(alpha,beta){
  J = nrow(Cum_Njd_A)
  LOG = 0
  for(j in 1:J){
    LOG_j = alpha * log (beta/(beta+Pj_A[[j,1]]*sum(PId_DF_A[1:(J-j+1)])))
    for(n in 1: Cum_Njd_A[[j,(J-j+1)]]){
      LOG_j = LOG_j + log((alpha+n-1)/(beta+Pj_A[[j,1]]*sum(PId_DF_A[1:(J-j+1)])))
    }
    LOG = LOG + LOG_j
  }
  return(-LOG)
...
# De Vylder's (DV) iterative procedure
...
Prior_Mom <- function(PriorParam,PJ){
  J = length(PJ)
  alpha = rep(0,J)
  beta = rep(0,J)
  for(j in 1:J){
