...
CMOM_RBNS_CP_A <- cmom_cp(D_A,Vt_DF_A,H_A_CP)
CMOM_IBNR_CP_A <- cmom_cp(D_A,Vt_DF_A,H_A_CP)
CMOM_CBNI_CP_A <- cmom_cp(D_A,Vt_DF_A,H_A_CP)

# Determine the moments based on the compound Poisson
MOM_RBNS_CP_A <- mom_rbns_gen(D_A,Njd_A,CMOM_RBNS_CP_A)
MOM_IBNR_CP_A <- mom_ibnr_gen(D_A,Pj_A,PId_DF_A,Post_Mom_DV_A,CMOM_IBNR_CP_A)
MOM_CBNI_CP_A <- mom_cbni_gen(D_A,PJ_A,PId_DF_A,Prior_Mom_DV_A,CMOM_CBNI_CP_A)
...
CMOM_RBNS_MULT_A <- cmom_mult(D_A,Vt_DF_A,G_A)
CMOM_IBNR_MULT_A <- cmom_mult(D_A,Vt_DF_A,G_A)
CMOM_CBNI_MULT_A <- cmom_mult(D_A,Vt_DF_A,G_A)

# Determine the moments based on the multinomial
MOM_RBNS_MULT_A <- mom_rbns_gen(D_A,Njd_A,CMOM_RBNS_MULT_A)
MOM_IBNR_MULT_A <- mom_ibnr_gen(D_A,Pj_A,PId_DF_A,Post_Mom_DV_A,CMOM_IBNR_MULT_A)
MOM_CBNI_MULT_A <- mom_cbni_gen(D_A,PJ_A,PId_DF_A,Prior_Mom_DV_A,CMOM_CBNI_MULT_A)
...
CMOM_RBNS_DIR_A <- cmom_dir(D_A,Vt_DF_A,G_A)
CMOM_IBNR_DIR_A <- cmom_dir(D_A,Vt_DF_A,G_A)
CMOM_CBNI_DIR_A <- cmom_dir(D_A,Vt_DF_A,G_A)

#Determine the moments based on the Dirichlet
MOM_RBNS_DIR_A <- mom_rbns_gen(D_A,Njd_A,CMOM_RBNS_DIR_A)
MOM_IBNR_DIR_A <- mom_ibnr_gen(D_A,Pj_A,PId_DF_A,Post_Mom_DV_A,CMOM_IBNR_DIR_A)
MOM_CBNI_DIR_A <- mom_cbni_gen(D_A,PJ_A,PId_DF_A,Prior_Mom_DV_A,CMOM_CBNI_DIR_A)
...
# Risk group B
# Determine the severity distribution moments based on the compound Poisson
CMOM_RBNS_CP_B <- cmom_cp(D_B,Vt_DF_B,H_B_CP)
CMOM_IBNR_CP_B <- cmom_cp(D_B,Vt_DF_B,H_B_CP)
CMOM_CBNI_CP_B <- cmom_cp(D_B,Vt_DF_B,H_B_CP)

# Determine the moments based on the compound Poisson
MOM_RBNS_CP_B <- mom_rbns_gen(D_B,Njd_B,CMOM_RBNS_CP_B)
MOM_IBNR_CP_B <- mom_ibnr_gen(D_B,Pj_B,PId_DF_B,Post_Mom_DV_B,CMOM_IBNR_CP_B)
MOM_CBNI_CP_B <- mom_cbni_gen(D_B,PJ_B,PId_DF_B,Prior_Mom_DV_B,CMOM_CBNI_CP_B)
...
# Determine the severity distribution moments based on the multinomial
CMOM_RBNS_MULT_B <- cmom_mult(D_B,Vt_DF_B,G_B)
CMOM_IBNR_MULT_B <- cmom_mult(D_B,Vt_DF_B,G_B)
CMOM_CBNI_MULT_B <- cmom_mult(D_B,Vt_DF_B,G_B)

# Determine the moments based on the multinomial
MOM_RBNS_MULT_B <- mom_rbns_gen(D_B,Njd_B,CMOM_RBNS_MULT_B)
MOM_IBNR_MULT_B <- mom_ibnr_gen(D_B,Pj_B,PId_DF_B,Post_Mom_DV_B,CMOM_IBNR_MULT_B)
MOM_CBNI_MULT_B <- mom_cbni_gen(D_B,PJ_B,PId_DF_B,Prior_Mom_DV_B,CMOM_CBNI_MULT_B)
...
#Assume that delta=1 so that the payment pattern for the Dirichlet deltat is the same as for the other models vt
#The delta chosen impacts the volatility of the model
CMOM_RBNS_DIR_B <- cmom_dir(D_B,Vt_DF_B,G_B)
CMOM_IBNR_DIR_B <- cmom_dir(D_B,Vt_DF_B,G_B)
CMOM_CBNI_DIR_B <- cmom_dir(D_B,Vt_DF_B,G_B)

#Determine the moments based on the Dirichlet
MOM_RBNS_DIR_B <- mom_rbns_gen(D_B,Njd_B,CMOM_RBNS_DIR_B)
MOM_IBNR_DIR_B <- mom_ibnr_gen(D_B,Pj_B,PId_DF_B,Post_Mom_DV_B,CMOM_IBNR_DIR_B)
MOM_CBNI_DIR_B <- mom_cbni_gen(D_B,PJ_B,PId_DF_B,Prior_Mom_DV_B,CMOM_CBNI_DIR_B)
...
Summary_mean <- data.frame("RBNS" = c(MOM_RBNS_MULT_A[1], MOM_RBNS_MULT_B[1]), 
                           "IBNR" = c(MOM_IBNR_MULT_A[1], MOM_IBNR_MULT_B[1]),
                           "CBNI" = c(MOM_CBNI_MULT_A[1], MOM_CBNI_MULT_B[1]))
